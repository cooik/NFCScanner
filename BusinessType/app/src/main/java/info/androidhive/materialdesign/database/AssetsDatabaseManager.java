...
/**
 * Created by yuqiao-liucs on 2016/3/17.
 */
import java.io.File;
...
public class AssetsDatabaseManager {
    ...
    private static String databasepath = "/data/data/info.androidhive.materialdesign/database"; // %s is packageName
    ...
    public SQLiteDatabase getDatabase(String dbfile) {
        if(databases.get(dbfile) != null){
            Log.i(tag, String.format("Return a database copy of info.androidhive.materialdesign", dbfile));
            return (SQLiteDatabase) databases.get(dbfile);
        }
        if(context==null)
            return null;

        Log.i(tag, String.format("Create database info.androidhive.materialdesign", dbfile));
